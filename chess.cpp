 ...
 *      CHESS
 * Authors:
 *      Br. Helfrich, Ashley DeMott, and Logan Huston 
 * Description: 
...
void callBack(Interface *pUI, void * p)
{
   ...
   if (pUI->getSelectPosition() != -1 && (board->getPieceAtPosition(getRC(pUI->getSelectPosition()))->isSpace() 
      || board->getCurrentTeam() != board->getPieceAtPosition(getRC(pUI->getSelectPosition()))->getIsWhite()))
         pUI->clearSelectPosition();
...
/***************************************
 * MAIN  
 * Just initialize my Demo type and 
 * call the display engine. That is all!
 *****************************************/
#ifdef _WIN32
#include <windows.h>
int WINAPI WinMain(
