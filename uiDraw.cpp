...
void ogstream::drawPiece(Piece* p) const
{
   ...
   text[0] = (p->getType());
   text[1] =(p->getIsWhite() ? 'w' : 'b');
   drawText(xGL, yGL, text);

   // TEST - draw one rectangle
   int x0 = 4; // 4
   int y0 = -3; // -3
   int x1 = -4; // -4
   int y1 = -3; // -3
   ...
   glVertex2i(xGL + x0, yGL + y0);
   glVertex2i(xGL + x1, yGL + y1);
   glVertex2i(xGL + x2, yGL + y2);
   glVertex2i(xGL + x3, yGL + y3);
