...
* Summary:
*		The base class for all types of Pieces in a chess game. This file 
*     contains general movements repeated among all pieces.
************************************************************************/
#pragma once
#include "Piece.h"
...
set <Move> Piece::getSlidingMoves(const Board* board, RC current, RC moves[])
{
   ...
   for (int i = 0; i < moveSize; i++)
   {
      ...
      {
         while (r >= 0 && r < 8 && c >= 0 && c < 8 && (*board).getPieceAtPosition(RC(r, c))->isSpace())
         {
         ...
         if (board->isValidPosition(RC(r, c)))
         {  
            // If position is empty or has a piece on the opposite team, add a possible move.
...
set <Move> Piece::getNonSlidingMoves(const Board* board, RC current, RC moves[]) 
{
   ...
   for (int i = 0; i < moveSize; i++)
   {
      ...
      if (board->isValidPosition(RC(r, c)))
      {
         // If the piece at the position is the opposite color, or a Space, then add a possible move
         if ((*board).getPieceAtPosition(RC(r, c))->isSpace() || (!this->isWhite && (*board).getPieceAtPosition(RC(r, c))->getIsWhite()))
            possible.insert(Move(RC(row, col), RC(r, c)));
         if ((*board).getPieceAtPosition(RC(r, c))->isSpace() || (this->isWhite && !(*board).getPieceAtPosition(RC(r, c))->getIsWhite()))
            possible.insert(Move(RC(row, col), RC(r, c)));
